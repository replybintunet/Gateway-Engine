...
type SheetView = "checkout" | "status" | "error";
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: "rgba(52,168,83,0.15)", pointerEvents: "none"
      }} />
      <div style={{
        position: "absolute", bottom: -20, left: -20,
        width: 80, height: 80, borderRadius: "50%",
        background: "rgba(52,168,83,0.08)", pointerEvents: "none"
      }} />

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        {/* Chip */}
        <div style={{
          width: 36, height: 28, background: "linear-gradient(135deg, #d4a843, #f0c060)",
          borderRadius: 5, border: "1px solid rgba(255,255,255,0.2)"
        }} />
        {brand && <CardBrandIcon brand={brand} />}
      </div>

      <div style={{ fontSize: 18, letterSpacing: 3, marginBottom: 18, opacity: card.number ? 1 : 0.5 }}>
        {displayNumber}
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: 1, opacity: card.name ? 1 : 0.5, textTransform: "uppercase" }}>
...
export default function App() {
  ...
  const [statusTitle, setStatusTitle] = useState("Awaiting M-Pesa PIN");
  const [statusDesc, setStatusDesc] = useState("Please check your phone handset for the active M-Pesa SIM popup window.");
  ...
  function startPolling(reference: string) {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        const res = await fetch(`payment.php?action=verify&reference=${encodeURIComponent(reference)}`);
        const result = await res.json();
        if (result.status === true && result.data) {
          if (result.data.status === "success") showSuccess();
          else if (result.data.status === "failed") showError(result.data.gateway_response);
        }
      } catch { /* silent */ }
    }, 2500);
  }
  ...
  async function processMpesa() {
    ...
    setStatusDesc("Please check your phone handset for the active M-Pesa SIM popup window.");
    setView("status");
    startCountdown();
    try {
      const res = await fetch("payment.php?action=charge", {
        method: "POST",
      ...
      catch { throw new Error("Malformed JSON response payload."); }
      if (result.status === true && result.data?.reference) {
        startPolling(result.data.reference);
      } else {
        showError(result.message || "Initialization rejected.");
      }
    } catch (err: unknown) {
  ...
  async function processCard() {
    ...
    if (!amount || raw.length < 13 || !card.expiry || !card.cvv || !card.name) {
      setErrorMsg("Please fill in all card details and amount.");
      setView("error");
      return;
    }
    setStatusTitle("Processing Card");
    setStatusDesc("Please wait while we securely process your card payment.");
    setView("status");
    startCountdown();
    try {
      const res = await fetch("payment.php?action=card", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amount,
      ...
      if (result.status === true && result.data?.reference) {
        ...
        showError(result.message || "Card charge failed.");
      }
    } catch (err: unknown) {
      showError(err instanceof Error ? err.message : "An error occurred.");
    }
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden", background: "#0b0f17" }}>
      ...
      <div style={{
        ...
        transform: `translate(-50%, ${view === "checkout" && isOpen ? "0%" : "110%"})`,
        ...
        <div style={{ width: 38, height: 4, background: "#e2e5ec", borderRadius: 2, margin: "0 auto 22px" }} />
        ...
        <p style={{ fontSize: 14, color: "#606770", margin: "0 0 20px" }}>
        ...
        <div style={{
          ...
          background: "#f4f6f9", borderRadius: 12, padding: 4
        }}>
          {(["mpesa", "card"] as PaymentMethod[]).map((m) => (
            <button
              ...
              style={{
                ...
                color: method === m ? "#1f2226" : "#606770",
                boxShadow: method === m ? "0 2px 8px rgba(0,0,0,0.1)" : "none",
              }}
            >
        ...
        <div style={{ background: "#fff", border: "1.5px solid #dcdfe6", borderRadius: 12, padding: "10px 16px", marginBottom: 16 }}>
          <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#606770", textTransform: "uppercase" }}>
            Amount (KES)
          </label>
          <input
            type="number"
            ...
            placeholder="e.g. 100"
            style={{ width: "100%", border: "none", background: "transparent", fontSize: 16, fontWeight: 600, outline: "none", color: "#1f2226" }}
          />
        </div>
        ...
        {method === "mpesa" && (
          <>
            ...
            <p style={{ fontSize: 12, color: "#909399", margin: "0 0 16px", lineHeight: 1.5 }}>
              Number format should start normally with 07 or 01. An STK push will be sent to your phone.
            </p>
          </>
        )}
        ...
        {method === "card" && (
          <>
            ...
            <div style={{ background: "#fff", border: "1.5px solid #dcdfe6", borderRadius: 12, padding: "10px 16px", marginBottom: 14 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#606770", textTransform: "uppercase" }}>
                Card Number {brand && <span style={{ color: "#34a853", marginLeft: 6 }}>{brand}</span>}
              </label>
              <input
                type="text"
                inputMode="numeric"
                ...
                maxLength={19}
                style={{ width: "100%", border: "none", background: "transparent", fontSize: 16, fontWeight: 600, outline: "none", color: "#1f2226", letterSpacing: 2 }}
              />
            </div>

            <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
              <div style={{ flex: 1, background: "#fff", border: "1.5px solid #dcdfe6", borderRadius: 12, padding: "10px 16px" }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#606770", textTransform: "uppercase" }}>
                  Expiry Date
                </label>
                <input
      ...
      <div style={{
        position: "fixed", bottom: 0, left: "50%",
        transform: `translate(-50%, ${view === "status" && isOpen ? "0%" : "110%"})`,
        width: "100%", maxWidth: 450,
        background: "#fff",
        borderTopLeftRadius: 28, borderTopRightRadius: 28,
        padding: "14px 24px 34px",
        ...
        <div style={{ width: 38, height: 4, background: "#e2e5ec", borderRadius: 2, margin: "0 auto 24px" }} />
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "10px 0 10px" }}>
          {!isSuccess ? (
            <div style={{
              ...
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 32, fontWeight: 700, color: "#1f2226" }}>
                {countdown}
              </span>
            </div>
          ) : (
            <div style={{
              width: 80, height: 80, background: "#e6f4ea", borderRadius: "50%",
              display: "flex", alignItems: "center", justifyContent: "center",
              color: "#34a853", marginBottom: 20
            }}>
          ...
          <h3 style={{ margin: "0 0 8px", fontFamily: "'Space Grotesk', sans-serif", fontSize: 24, color: "#1f2226" }}>
            {statusTitle}
          </h3>
      ...
      <div style={{
        position: "fixed", bottom: 0, left: "50%",
        transform: `translate(-50%, ${view === "error" && isOpen ? "0%" : "110%"})`,
        width: "100%", maxWidth: 450,
        background: "#fffdfd",
        borderTopLeftRadius: 28, borderTopRightRadius: 28,
        ...
        <div style={{ width: 38, height: 4, background: "#e2e5ec", borderRadius: 2, margin: "0 auto 20px" }} />
        <div style={{
          width: 48, height: 48, background: "#fce8e6", borderRadius: "50%",
          display: "flex", alignItems: "center", justifyContent: "center",
          color: "#d93025", marginBottom: 16
        }}>
        ...
        <h3 style={{ margin: "0 0 8px", fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, color: "#1f2226" }}>
          Transaction Failed
        </h3>
